...
r14 = Review.create(date_visited: "2022-09-09", rating: 4, review: "Pretty damn good. Be warned, hot sauce is HOT.", hot_sauce_spice: 5, user_id: emilio.id, cart_id: usa.id)
r15 = Review.create(date_visited: "2022-11-02", rating: 4, review: "Apparently a Vendy Award winner. It shows. Lines can wrap around the block on a regular weekday!", hot_sauce_spice: 4, user_id: ethan.id, cart_id: adel.id)
r16 = Review.create(date_visited: "2021-08-28", rating: 4, review: "Super high quality but not pretentious. I've enjoyed this platter many a summer night in front of Lincoln Center.", hot_sauce_spice: 3, user_id: ethan.id, cart_id: casbah.id)
...
Comment.create(comment: "Those Fordham students don't know how good they got it", review_id: r8.id, user_id: emilio.id)
Comment.create(comment: "When it's good, it's GOOOOOOD", review_id: r7.id, user_id: wendy.id)
Comment.create(comment: "I love finding a whole cardamom in the rice. That's when you know...", review_id: r7.id, user_id: justin.id)
Comment.create(comment: "Whoaaa where was my invite!?", review_id: r16.id, user_id: justin.id)
...
LikesReview.create(user_id: justin.id, review_id: r7.id)
LikesReview.create(user_id: emilio.id, review_id: r7.id)
...
LikesReview.create(user_id: wendy.id, review_id: r14.id)
LikesReview.create(user_id: ethan.id, review_id: r14.id)
LikesReview.create(user_id: emilio.id, review_id: r8.id)
LikesReview.create(user_id: ethan.id, review_id: r8.id)
LikesReview.create(user_id: justin.id, review_id: r16.id)
